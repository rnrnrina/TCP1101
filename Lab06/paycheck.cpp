...
int main()
{
    ...
    cout << endl
         << endl;
    computePaycheck(payRate, hours, grossPay, netPay); // ---Calculate payment
...
void computePaycheck(float rate, int time, float &gross, float &net)
{
    ...
    gross = static_cast<float>(rate) * static_cast<float>(time);
    net = (gross * (100-15))/100;

    // ---6.2.4 Alter the program so that gross and net are printed in the function compute computePaycheck instead of in main() . The main() function executes the statement
    // Fill in the code to output grossPay
